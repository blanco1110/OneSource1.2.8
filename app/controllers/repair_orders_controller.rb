class RepairOrdersController < ApplicationController
  ...
  private
    ...
    def repair_order_params
      params.require(:repair_order).permit(:repair_order_status_id, :repair_order_date, :repair_order_subtotal, :repair_order_tax, :repair_order_total, :repair_order_accessories, :repair_order_notes,:repair_order_status_date, :customer_id, customer_attributes: [:id, :customer_fname, :customer_lname, :customer_email, :customer_phone, :customer_call, :customer_text, :customer_zip,],:devices_attributes => [:id, :_destroy, :imei_number, :device_version_id, :device_type, :device_color, :device_notes, :repair_order_id, :device_pre_tests_attributes => [:id, :_destroy, :device_id, :test_id, :pre_test_condition], :device_post_tests_attributes => [:id, :_destroy, :device_id, :test_id, :post_test_condition],:repair_order_items_attributes => [:id, :_destroy, :device_component_id, :technician_id, :warranty_expire_date, :warranty_coverage_notes, :repaired_as_warranty, :warranty_repair_date, :price, :device_id]])
    end
end
